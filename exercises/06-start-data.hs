...
succBitVector End = End:.BOne
succBitVector (bs:.BZero) = bs:.BOne
succBitVector (bs:.BOne)  = succBitVector bs :. BZero
...
canonicalise End = End
canonicalise (bs:.b) = (if isEnd bs then End else canonicalise bs) :. b
-- HINT:
...
integerToBitVector 0 = End
integerToBitVector n = integerToBitVector (n`div`2) :. toBit (n`mod`2)
  where toBit 0 = BZero
        toBit _ = BOne
...
bitVectorToInteger bv = toInt $ canonicalise bv
  where fromBit BZero = 0
        fromBit BOne  = 1
        toInt End = 0
        toInt (bs:.b) = toInt bs * 2 + fromBit b
...
addBitVector End as  = as
addBitVector bs  End = bs
addBitVector (bs:.BOne)  (as:.BOne)  = succBitVector (addBitVector bs as) :. BZero
addBitVector (bs:.BZero) (as:.BZero) = addBitVector bs as :. BZero
addBitVector (bs:._) (as:._) = addBitVector bs as :. BOne
