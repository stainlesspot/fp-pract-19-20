...
-- Parses a whole number.  asciiToDigit is defined at the bottom of the file, it's not very interesting.
--
-- EXAMPLES:
-- > parse number "123"
-- Just 123
...
numberParser = fmap Number number
...
--stringParser = String <$> surround (char '"') (many nom)
stringParser = fmap String quotedString

quotedString :: Parser String
quotedString = surround (char '"') (many $ satisfy $ not . (=='"')) 
...
-- EXERCISE: A parser that runs both parsers, but ignores the result of the right one.
...
sepBy p sep = sepBy1 p sep <|> result []
...
objectParser
  = fmap Object
  $ between (char '{') (char '}') 
  $ sepBy objectElementParser (char ',')
-- * Technical reasons:
